...
import com.ethan.twfaith.commands.methods.CreateFaith;
...
public class FaithCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            File senderFaithFile = new File(faith_folder, player.getUniqueId() + ".json");
            ...
            if (Objects.equals(args[0], "list") && args.length == 1){
                try{
                    ...
                    for (File file : Objects.requireNonNull(faith_folder.listFiles())){
                        ...
                        player.sendMessage(read_faith.getFaithName());
                    }
            ...
            if (Objects.equals(args[0], "join") && args.length == 2){
                ...
                for (File faith_file : Objects.requireNonNull(faith_folder.listFiles())){
                    try{
                        ...
                        Faith read_faith = TWFaith.getGson().fromJson(faith_reader, Faith.class);
                        List<UUID> invited_players = read_faith.getInvitedPlayers();
                        List<UUID> followers = read_faith.getFollowers();
                        System.out.println("file detected");
                        if (read_faith.getFaithName().equals(args[1]) && read_faith.getInvitedPlayers().contains(player.getUniqueId()) && !followers.contains(player.getUniqueId())){
                            invited_players.remove(player.getUniqueId());
                            followers.add(player.getUniqueId());
                            player.sendMessage("You have become a follower of " + read_faith.getFaithName());
                            read_faith.setInvitedPlayers(invited_players);
                            read_faith.setFollowers(followers);

                            if (Objects.requireNonNull(Bukkit.getPlayer(read_faith.getLeader())).isOnline()){
                                Objects.requireNonNull(Bukkit.getPlayer(read_faith.getLeader())).sendMessage(player.getDisplayName() + " has become a follower of " + read_faith.getFaithName());
                            }
                            ...
                            playerData.setLed_by(read_faith.getLeader());
            ...
            if (Objects.equals(args[0], "leave")){
                ...
                try{
                    if (playerData.getIn_faith()){
                        ...
                        File faith_file = new File (faith_folder, playerData.getLed_by() + ".json");
                        Reader faith_reader = new FileReader(faith_file);
                        Faith read_faith = TWFaith.getGson().fromJson(faith_reader, Faith.class);
                        List<UUID> followers = read_faith.getFollowers();
                        ...
                        playerData.setIn_faith(false);
                        playerData.setLeader(false);
                        playerData.setFaith("");
                        playerData.setLed_by(null);
                        PlayerHashMap.player_data_hashmap.put(player.getUniqueId(), playerData);
